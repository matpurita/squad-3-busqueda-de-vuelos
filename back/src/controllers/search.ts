  ...
  try {
    ...
    if (searchParams.passengers) {
      departureSeats = departureSeats.filter(
        (seat: any) => seat.flight.seats.length >= searchParams.passengers
      )
      returnSeats =
        returnSeats?.filter((seat: any) => seat.flight.seats.length >= searchParams.passengers) ??
        null
    }
...
async function getFlightSuggestions(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json({
      suggestions: suggestions.map((flight: any) => ({
        id: flight.id,
